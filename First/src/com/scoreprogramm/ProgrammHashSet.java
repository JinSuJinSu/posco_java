	...
	public static void main(String[] args) {
		...
		while (running) { // 종료전까지 계속 반복
			...
			case 1:
				...
				studentSet.add(student);// hashset에 데이터 추가
				break;
			...
			case 3:
				...
				for (Student students : studentSet) {
					if (students.getName().equals(deleteName)) { // 학생 이름이 같아야 맞는 정보를 찾을 수 있다.
						studentSet.remove(students); // 학생 데이터 제거
...
//						}
			...
			case 4:
				...
				for (Student studentInformation : studentSet) { // for문으로 모든 정보를 출력한다.
					System.out.printf("이름:%s\t 국어:%d\t 영어:%d\t 수학:%d\t 합계:%d\t 평균:%.2f%n", studentInformation.getName(),
							studentInformation.getLanguageScore(), studentInformation.getEnglishScore(),
							studentInformation.getMathScore(), studentInformation.getSumScore(),
							studentInformation.getAvgScore());
				}
				break;
